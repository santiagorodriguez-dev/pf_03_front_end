...
st.text_area("Respuesta del asistente:", value="\n".join(st.session_state.historial), height=350, disabled=True)

# Entrada de texto con clave dinámica
user_input = st.text_input("Introduce la conversacion telefonica, para poder obetener las conclusiones:", key=f"user_input_{st.session_state.input_key}")
...
with col1:
    if st.button("Enviar"):
        if user_input.strip():  # Evita entradas vacías
            st.session_state.historial.append(f"Tú: {user_input}")
